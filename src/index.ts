...
container.style.bottom = '64px'; // padding from bottom
container.style.right = '64px'; // padding from the right
container.style.zIndex = '1000'; // Ensure the icon is above other elements

// Creates a div element for the icon
const icon = document.createElement('div');

// Sets the inner HTML of the icon to the SVG content
icon.innerHTML = svgContent;

// Sets CSS properties for the icon
icon.style.width = '40px'; //  This sets the width of the icon
icon.style.height = 'auto'; //  This maintains aspect ratio
icon.style.cursor = 'pointer'; // This changes the cursor while hoovering over the icon

// event listener to do something when clicked
icon.addEventListener('click', () => {
  alert('Icon clicked!');
});

// Sets a class for the icon for additional styling
...
function setSVGColor(svgElement: SVGElement, color: string): void {
  svgElement.querySelectorAll('path, circle, rect, polygon').forEach((el: Element) => {
    if (el instanceof SVGGraphicsElement) {
      el.style.fill = color; // Set fill color
    }
  });
}

// This is a function to change the color of the SVG icon
function changeIconColor(color: string): void {
  // Gets the SVG element inside the icon
  const svgElement: SVGSVGElement | null = icon.querySelector('svg');
  if (svgElement) {
    // Applys the color change function to the SVG
    setSVGColor(svgElement, color);
  }
}

// Changes color of the SVG Icom
changeIconColor('#FF0000'); // Changes icon color to red

// Optional: Add an event listener if you want the icon to do something when clicked
icon.addEventListener('click', () => {
  // Your logic here, for example:
  alert('Icon clicked!');
});

// Set a class for the icon for additional styling if needed
icon.className = 'custom-icon';

// Append the image to the container instead of the JSON string
container.appendChild(icon);
...
/* To do:
surround the SVG icon with a container
that can be changed to round or square from the script.
Be able to change position from the script 
and be able to change the color of the icon from the script. */
